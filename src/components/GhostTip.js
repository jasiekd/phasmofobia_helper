  ...
  return (
    <span className="ghost-tip">
      ...
      {ghost.evidences.map((value) => {
        return <div className={isForcedEvidence(value) ? "forcedEvidence" : null}>- {t(value)}</div>;
      })}
      <div>{t("attack")}: {ghost.hunt}</div>
      <div>{t("specialMovement")}: {ghost.specialMovement ? t("yes") : t("no")}</div>
    </span>
  );
}
